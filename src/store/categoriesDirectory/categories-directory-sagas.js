...
import {
  fetchCategoriesDirectorySuccess,
...
function* setTheCategoyTitle(action) {
  const {
    payload: { nameDoc, titleName, allCategories },
  } = action;
...
function* setTheCategoryImage(action) {
  const {
    payload: { categoryName, imageInput },
  } = action;
  ...
  try {
    ...
    yield all(
      titleArr.map((title, index) => {
        return all([
          call(updateFieldOnDb, "directory", title, "id", index + 1),
          call(updateFieldOnDb, "categories", title, "id", `0${index + 1}`),
        ]);
      })
    );
...
function* setAddNewCategory(action) {
  const {
    payload: { data, categoriesData },
  } = action;

  for (let i = 0; i < categoriesData.length; i++) {
    if (categoriesData[i].id === data.id)
      return alert("The ID of the Element already exist");
  }
...
function* setChangeElementOrder(action) {
  const {
    payload: { categories, category, changeOrderInput },
  } = action;
  ...
  try {
    ...
    const elementToChangeOrder = categories.filter(
      (value) => value.id === orderInput
    );

    if (elementToChangeOrder.length === 0)
      return alert("The Id of the Element dosen`t exist");
    ...
    yield updateFieldOnDb(
      "directory",
      elementToChangeOrder[0].title,
      "id",
      category.id
    ); //2(1) 2->1
...
function* onSetChangeElmentOrder() {
  yield takeLatest(
    CATEGORIES_DIRECTORY_TYPES.SET_CHANGE_ELEMENT_ORDER_START,
    setChangeElementOrder
  );
}
function* onGetCategoriesFromDb() {
  yield takeLatest(
    CATEGORIES_DIRECTORY_TYPES.FETCH_CATEGORIES_DIRECTORY_START,
    getCategoriesFromDb
  );
}
...
function* onSetCategoryTitle() {
  yield takeLatest(
    CATEGORIES_DIRECTORY_TYPES.SET_CATEGORIES_NAME_START,
    setTheCategoyTitle
  );
}
...
function* onSetCategoryImage() {
  yield takeLatest(
    CATEGORIES_DIRECTORY_TYPES.SET_CATEGORIES_IMAGE_START,
    setTheCategoryImage
  );
}
function* onSetRemoveCategory() {
  yield takeLatest(
    CATEGORIES_DIRECTORY_TYPES.SET_CATEGORIES_REMOVE_START,
    setRemoveCategory
  );
}
...
function* onSetAddNewCategory() {
  yield takeLatest(
    CATEGORIES_DIRECTORY_TYPES.SET_ADD_NEW_CATEGORY_START,
    setAddNewCategory
  );
}
