...
const ItemAddProduct = ({ item }) => {
  ...
  const newBtnEvent = () => {
    if (!currentUser)
      return alert(
        `Only the administrator can add a new item,For testing, you can use email: admin@gmail.com password: admin1234`
      );
  ...
  return (
    <ProductCarTConatiner>
      <img
        alt="product"
        src="https://images.pexels.com/photos/1884581/pexels-photo-1884581.jpeg?cs=srgb&dl=pexels-bohlemedia-1884581.jpg&fm=jpg"
      ></img>
      {!newProductBtnClicked ? (
        <Button onClick={newBtnEvent} typebutton="inverted">
          {" "}
          New Product
        </Button>
      ) : (
        <AddNewProductForm
          item={item}
          newProductBtnClicked={newProductBtnClicked}
          setNewProductBtnClicket={setNewProductBtnClicket}
        ></AddNewProductForm>
      )}
    </ProductCarTConatiner>
  );
